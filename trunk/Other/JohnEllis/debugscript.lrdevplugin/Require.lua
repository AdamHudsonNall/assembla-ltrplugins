...
return LrFunctionContext.callWithContext ("", function (context)
    ...
    local f, e = io.open (filePath)
    if not f then raiseError (e) end
    local script, e = f:read ("*a")
    if not script then raiseError (e) end
    f:close ()

    local chunk, e = loadstring (script, filename)
